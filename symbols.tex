...
\newglossaryentry{symb:Div}{
    type=symbols,
    name={$\div$, :, /},
    description={Symbol für die Division. Gelegentilich wird \enquote{/} benutzt, insbesondere in Texten in denen keine Brüche gesetzt werden können sowie auf vielen Taschenrechnern. In deutschen Büchern (und auch diesem Buch) häufig \enquote{:}.
    },
    sort={Division}
}
...
\newglossaryentry{symb:folgt}{
    type=symbols,
    name={$\Rightarrow$},
    description={daraus folgt, z.B. $4 | n \Rightarrow 2 | n$},
    sort={folgt}
}
...
\newglossaryentry{symb:LeereMenge}{
    name={$\varnothing$, $\emptyset$, $\{ \; \}$},
    description={Die Leere Menge enthält kein Element},
    type=symbols,
    sort={leere Menge}
}
...
\newglossaryentry{symb:qed}{
    type=symbols,
    name={$\square,\blacksquare$},
    description={\enquote{Quot erat demonstrandum}, lat. \enquote{Was zu zeigen war}, d.h. Ende eines Beweises, Argumentes, Beispiels,...},
    sort={quod erat demonstrandum}
}

\newglossaryentry{symb:rReelleZahlen}{
    type=symbols,
    name={$\mathbb{R}$},
    description={die Menge der reellen Zahlen, insbesondere nutzen wir $\mathbb{R}$ für Strecken, $\mathbb{R}^2$ für Flächeninhalt und $\mathbb{R}^3$ für Volumen.}
}
...
\newglossaryentry{symb:Tupel}{
    type=symbols,
    name={$(x|y)$,$(x|y|z)$},
    description={\textbf{Tupel}, eine Folge von Zahlen. Insbesondere bezeichnen wir mit zweistelligen Tupeln (Paaren) Punkte in der 2-dimensionalen Fläche und mit dreistelligen Tupeln (Tripeln) Punkte im 3-dimensionalen Raum. Übliche Schreibweisen sind auch $(x,y)$ und $(x;y)$, gelegentlich auch mit geschweiften oder eckigen Klammern wie $\{x,y\}$ oder $[x,y]$},
    sort={Tupel}
}
